...
public abstract class FCommand extends MCommand<P> {
	...
	public boolean validSenderType(CommandSender sender, boolean informSenderIfNot) {
		...
		if (!(this.senderMustBeMember || this.senderMustBeModerator || this.senderMustBeCoLeader || this.senderMustBeLeader)) {
			return true;
		}
		...
		if (this.senderMustBeLeader && !fme.getRole().isAtLeast(Role.LEADER)) {
	...
	public Faction strAsFaction(String name, Faction def, boolean msg) {
		...
		if (name != null) {
			...
			Faction faction = Factions.getInstance().getByTag(name); // Checks
																		// for
																		// faction
																		// name
																		// match.

			// Now lets try for warzone / safezone. Helpful for custom warzone /
			// safezone names.
			// Do this after we check for an exact match in case they rename the
			// warzone / safezone
