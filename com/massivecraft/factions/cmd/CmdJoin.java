...
public class CmdJoin extends FCommand {
	...
	public void perform() {
		...
		if (faction.hasMoreMembers()) {
			if (faction.getFPlayers().size() >= Conf.upgradedFactionMemberLimit) {
				msg(TL.COMMAND_JOIN_ATLIMIT, faction.getTag(fme), Conf.upgradedFactionMemberLimit, fplayer.describeTo(fme, false));
				return;
			}
		} else {
			if (faction.getFPlayers().size() >= Conf.defaultFactionMemberLimit) {
				msg(TL.COMMAND_JOIN_ATLIMIT, faction.getTag(fme), Conf.defaultFactionMemberLimit, fplayer.describeTo(fme, false));
				return;
			}
		}
		...
		fme.addNewFaction(faction.getTag());
