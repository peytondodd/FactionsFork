...
public class CmdMod extends FCommand {
	...
	public void perform() {
		...
		if (you == null) {
			...
			for (FPlayer player : myFaction.getFPlayersWhereRole(Role.MEMBER)) {
				String s = player.getName();
				msg.then(s + " ").color(ChatColor.WHITE).tooltip(TL.COMMAND_MOD_CLICKTOPROMOTE.toString() + s)
						.command("/" + Conf.baseCommandAliases.get(0) + " mod " + s);
			}
